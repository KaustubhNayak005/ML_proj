...
We designed and implemented a from-scratch native-PyTorch layer named `CamouflageGATConv`. This layer improves upon the standard Graph Attention Network (GAT) by introducing a **similarity-weighted attention mechanism**. 
...
### Analysis
The results suggest our custom `CamouflageGAT` improves upon the standard `GAT` baseline specifically (**+3.6% absolute PR-AUC**). While GraphSAGE (which uses mean-aggregation rather than attention) remained the strongest overall architecture for this specific dataset, our custom mechanism successfully patched the vulnerability inherent in attention-based aggregation on fraud graphs.

**Limitations**: Although our preprocessing extracts per-relation edge indices (e.g. sharing a card versus sharing an email domain), the current model implementation combines these into a single flattened edge index as a scope simplification. The model cannot currently distinguish between different types of relationships.
